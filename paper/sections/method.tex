...
the current control gradient.

\subsection{Control-guided admission}

Our hard admission rule is
\begin{equation}
...
non-negative cosine similarity.  Crucially, Eq.~\ref{eq:hard-routing} replaces
...
\end{equation}
Here $\beta=0$ recovers hard routing and $\beta=1$ recovers the raw prediction
gradient.  This residual path cannot guarantee that only physical information
is retained; it tests whether hard routing closes learning channels needed by
...
the norm produced by Eq.~\ref{eq:hard-routing}.  A retention-matched shuffled
