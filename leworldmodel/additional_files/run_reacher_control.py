#!/usr/bin/env python3
"""Run matched clean/tagged JEPA and aligned-gradient arms on DMC Reacher."""
...
def load_reacher_configs() -> dict:
    """Build a four-arm Reacher grid without PushT-only pair diagnostics."""
...
def main() -> int:
    base_runner.CASE = "lewm-reacher-control"
    base_runner.load_configs = load_reacher_configs
    base_runner.render_figures = lambda args: print(
        "[figures] skipped for Reacher grid; metrics.jsonl is the source of truth"
    )
    os.environ.setdefault("RESULTS_DIR", "./results/reacher-control")
