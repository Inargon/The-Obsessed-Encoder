#!/usr/bin/env python3
"""Run matched clean/tagged JEPA and aligned-gradient arms on OGBench Cube."""
...
def load_cube_configs() -> dict:
    """Build a four-arm Cube grid without carrying PushT-only pair diagnostics."""
...
def main() -> int:
    base_runner.CASE = "lewm-cube-control"
    base_runner.load_configs = load_cube_configs
    base_runner.render_figures = lambda args: print(
        "[figures] skipped for Cube grid; metrics.jsonl is the source of truth"
    )
    os.environ.setdefault("RESULTS_DIR", "./results/cube-control")
