...
def test_control_grid_has_relation_core_sweep():
    ...
    for name, arm in config["arms"].items():
        ...
        expected_mode = (
            "masked_reachability"
            if name.startswith("masked_sequence_") or name == "effect_geometry_pred03"
            else name
        )
    ...
    assert "+loss.effect_geometry.enabled=true" in config["arms"]["effect_geometry_pred03"]["overrides"]
