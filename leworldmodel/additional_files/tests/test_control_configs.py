...
def test_control_grid_has_relation_core_sweep():
    ...
    for name, arm in config["arms"].items():
        ...
        expected_mode = (
            "masked_reachability"
            if name.startswith("masked_sequence_") or name == "action_patch_pred03"
            else name
        )
    ...
    router = config["arms"]["action_patch_pred03"]
    assert "+loss.pred_weight=0.3" in router["overrides"]
    assert any("ActionPatchRouter" in item for item in router["overrides"])
    assert "+loss.control.inverse_target=sequence" in router["overrides"]
