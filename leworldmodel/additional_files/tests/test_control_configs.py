...
def test_control_grid_has_relation_core_sweep():
    ...
    for name, arm in config["arms"].items():
        ...
        expected_mode = (
            "masked_reachability"
            if name.startswith("masked_sequence_") or name == "conflict_routed_pred03"
            else name
        )
    ...
    conflict = config["arms"]["conflict_routed_pred03"]["overrides"]
    assert "+loss.pred_weight=0.3" in conflict
    assert "+loss.gradient_routing.enabled=true" in conflict
    assert "+loss.control.inverse_target=sequence" in conflict
