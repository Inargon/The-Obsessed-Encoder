...
def _parse_dataset_str(dataset_str: str):
    ...
    # >>> obsessed-encoder: register the local ImageNet-1k parquet dataset (the
    # watermark seam lives on it); lazy import so the dinov3 package stays loadable
    # without the additional_files on sys.path.
    elif name == "ImageNet1kParquet":
        from additional_files.imagenet1k_parquet import ImageNet1kParquet

        class_ = ImageNet1kParquet
        if "split" in kwargs:
            kwargs["split"] = ImageNet1kParquet.Split[kwargs["split"]]
    # <<< obsessed-encoder
