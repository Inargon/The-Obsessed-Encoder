...
def build_data_loader_from_cfg(
    ...
    # >>> obsessed-encoder: carry the per-sample label through the collate so
    # the passive online probe in do_train has supervision (env-gated; the default
    # path keeps the upstream label-dropping collate).
    collate_base = collate_data_and_cast
    if os.environ.get("DINOV3_PROBE") == "1":
        from additional_files.probe_collate import collate_data_and_cast_with_labels

        collate_base = collate_data_and_cast_with_labels
    # <<< obsessed-encoder
    collate_fn = partial(
        collate_base,
    ...
    data_loader = make_data_loader(
        ...
        # >>> obsessed-encoder: re-enable the cyclic garbage collector inside
        # dataloader workers (see _enable_gc_in_workers); gated with the observer
        # so the default invocation keeps the upstream loader bit for bit.
        worker_init_fn=(_enable_gc_in_workers
                        if os.environ.get("DINOV3_PROBE") == "1" else None),
        # <<< obsessed-encoder
    )
    return data_loader
...
# >>> obsessed-encoder: do_train disables the cyclic garbage collector and
# manually collects every 150 iterations -- in the MAIN process only. Dataloader
# workers fork from that state, inherit the disabled collector, and (under the
# infinite sampler) live for the whole run, so per-item reference cycles from
# the decode/transform path accumulate without bound (measured ~6 GiB of worker
# RSS per 1000 batches of 128; flat with the collector re-enabled). Re-enabling
# GC inside each worker restores the plain-Python default there; the training
# loop's manual-GC policy is untouched.
...
def do_train(cfg, model, resume=False):
    ...
    # >>> obsessed-encoder: step budget -- DINOV3_MAX_ITER caps the run while
    # cfg.optim.epochs keeps setting the LR/WD/momentum schedule horizon; the two
    # are decoupled so a run can stop mid-schedule independently of the schedule
    # shape. Unset => upstream (the cap never engages).
    _max_iter_cap = int(os.environ.get("DINOV3_MAX_ITER", "0") or "0")
    if _max_iter_cap > 0:
        max_iter = min(max_iter, _max_iter_cap)
    # <<< obsessed-encoder
    if cfg.multidistillation.enabled:
    ...
    # >>> obsessed-encoder: the open-source release stripped the eval harness
    # and has no wandb, so all measurement (passive online probe + periodic val
    # pass + pair metrics + step-0 stimulus dump + the local metrics mirror) is
    # net-new and lives in additional_files/train_hooks.py. Env-gated; off by default
    # so the upstream loop is unchanged.
    from additional_files.train_hooks import observer_enabled, RunObserver

    run_observer = RunObserver(cfg, model) if observer_enabled() else None
    if run_observer is not None and start_iter == 0:
        run_observer.step0_dump()
    if run_observer is not None and start_iter > 0:
        # Probe continuity: restore the probe sidecar saved alongside the
        # checkpoint, so a resumed session's probe does not re-train from scratch
        # (the reset dip would be indistinguishable from a real probe drop).
        run_observer.load_sidecar(cfg.train.output_dir)
    # <<< obsessed-encoder
    ...
    for data in metric_logger.log_every(
        ...
        # >>> obsessed-encoder: passive observer (probe step + periodic val pass
        # + metrics log). Reads the detached teacher CLS stashed in forward_backward.
        if run_observer is not None:
            run_observer.on_step(
                iteration,
                model,
                data,
                dict(metrics_dict, total_loss=total_loss),
                scalars={"sched/lr": lr, "sched/wd": wd, "sched/mom": mom},
            )
        # <<< obsessed-encoder
        ...
        if (iteration + 1) % cfg.checkpointing.period == 0:
            ...
            if distributed.is_subgroup_main_process():
                ...
                # >>> obsessed-encoder: the probe sidecar rides every checkpoint
                # write, so upstream's native resume also restores the probe.
                if run_observer is not None:
                    run_observer.save_sidecar(cfg.train.output_dir)
                # <<< obsessed-encoder
