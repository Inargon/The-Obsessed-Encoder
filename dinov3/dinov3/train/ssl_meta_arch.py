...
class SSLMetaArch(nn.Module):
    ...
    def forward_backward(
        ...
        # >>> obsessed-encoder: stash the detached teacher CLS (mean over the 2
        # global crops) for the passive online linear probe in do_train. Pure
        # observer -- it never enters the SSL loss or the backward graph -- and
        # gated on the observer's env switch so the ungated path is upstream's.
        # (import kept local so the whole change lives inside this block)
        import os as _os

        if _os.environ.get("DINOV3_PROBE") == "1":
            self.last_teacher_cls = teacher_global["cls_pre_head"].mean(0).detach()
        # <<< obsessed-encoder
